import { Box, Button, IconButton, Typography, useTheme, useMediaQuery } from "@mui/material";
import { tokens } from "../../theme";
import { mockTransactions } from "../../data/mockData";
import DownloadOutlinedIcon from "@mui/icons-material/DownloadOutlined";
import EmailIcon from "@mui/icons-material/Email";
import PointOfSaleIcon from "@mui/icons-material/PointOfSale";
import PersonAddIcon from "@mui/icons-material/PersonAdd";
import TrafficIcon from "@mui/icons-material/Traffic";
import Header from "../../components/Header";
import LineChart from "../../components/LineChart";
import GeographyChart from "../../components/GeographyChart";
import BarChart from "../../components/BarChart";
import StatBox from "../../components/StatBox";
import ProgressCircle from "../../components/ProgressCircle";

const Dashboard = () => {
  const theme = useTheme();
  const colors = tokens(theme.palette.mode);
  const isSmallScreen = useMediaQuery(theme.breakpoints.down('sm'));
  const isMediumScreen = useMediaQuery(theme.breakpoints.down('md'));

  return (
    <Box m={isSmallScreen ? "10px" : "20px"}>
      {/* HEADER */}
      <Box 
        display="flex" 
        flexDirection={isSmallScreen ? "column" : "row"} 
        justifyContent="space-between" 
        alignItems="center" 
        mb={isSmallScreen ? "10px" : "20px"}
      >
        <Header 
          title={isSmallScreen ? "DASH" : "DASHBOARD"} 
          subtitle={isSmallScreen ? "" : "Welcome to your dashboard"} 
        />

        <Button
          sx={{
            backgroundColor: colors.blueAccent[700],
            color: colors.grey[100],
            fontSize: isSmallScreen ? "12px" : "14px",
            fontWeight: "bold",
            padding: isSmallScreen ? "8px 16px" : "10px 20px",
          }}
        >
          <DownloadOutlinedIcon sx={{ mr: "10px", fontSize: isSmallScreen ? "20px" : "26px" }} />
          {isSmallScreen ? "Download" : "Download Reports"}
        </Button>
      </Box>

      {/* GRID & CHARTS */}
      <Box
        display="grid"
        gridTemplateColumns={isSmallScreen ? "repeat(1, 1fr)" : (isMediumScreen ? "repeat(2, 1fr)" : "repeat(12, 1fr)")}
        gridAutoRows="140px"
        gap={isSmallScreen ? "10px" : "20px"}
      >
        {/* ROW 1 */}
        <Box
          gridColumn={isSmallScreen ? "span 1" : "span 3"}
          backgroundColor={colors.primary[400]}
          display="flex"
          alignItems="center"
          justifyContent="center"
          p={isSmallScreen ? "10px" : "20px"}
        >
          <StatBox
            title={isSmallScreen ? "12k" : "12,361"}
            subtitle="Emails Sent"
            progress="0.75"
            increase="+14%"
            icon={
              <EmailIcon
                sx={{ color: colors.greenAccent[600], fontSize: isSmallScreen ? "20px" : "26px" }}
              />
            }
          />
        </Box>
        <Box
          gridColumn={isSmallScreen ? "span 1" : "span 3"}
          backgroundColor={colors.primary[400]}
          display="flex"
          alignItems="center"
          justifyContent="center"
          p={isSmallScreen ? "10px" : "20px"}
        >
          <StatBox
            title={isSmallScreen ? "431k" : "431,225"}
            subtitle="Sales Obtained"
            progress="0.50"
            increase="+21%"
            icon={
              <PointOfSaleIcon
                sx={{ color: colors.greenAccent[600], fontSize: isSmallScreen ? "20px" : "26px" }}
              />
            }
          />
        </Box>
        <Box
          gridColumn={isSmallScreen ? "span 1" : "span 3"}
          backgroundColor={colors.primary[400]}
          display="flex"
          alignItems="center"
          justifyContent="center"
          p={isSmallScreen ? "10px" : "20px"}
        >
          <StatBox
            title={isSmallScreen ? "32k" : "32,441"}
            subtitle="New Clients"
            progress="0.30"
            increase="+5%"
            icon={
              <PersonAddIcon
                sx={{ color: colors.greenAccent[600], fontSize: isSmallScreen ? "20px" : "26px" }}
              />
            }
          />
        </Box>
        <Box
          gridColumn={isSmallScreen ? "span 1" : "span 3"}
          backgroundColor={colors.primary[400]}
          display="flex"
          alignItems="center"
          justifyContent="center"
          p={isSmallScreen ? "10px" : "20px"}
        >
          <StatBox
            title={isSmallScreen ? "1.3M" : "1,325,134"}
            subtitle="Traffic Received"
            progress="0.80"
            increase="+43%"
            icon={
              <TrafficIcon
                sx={{ color: colors.greenAccent[600], fontSize: isSmallScreen ? "20px" : "26px" }}
              />
            }
          />
        </Box>

        {/* ROW 2 */}
        <Box
          gridColumn={isSmallScreen ? "span 1" : "span 8"}
          gridRow="span 2"
          backgroundColor={colors.primary[400]}
          p={isSmallScreen ? "10px" : "20px"}
        >
          <Box
            display="flex"
            flexDirection={isSmallScreen ? "column" : "row"}
            justifyContent="space-between"
            alignItems="center"
            mb={isSmallScreen ? "10px" : "25px"}
          >
            <Box>
              <Typography
                variant={isSmallScreen ? "h6" : "h5"}
                fontWeight="600"
                color={colors.grey[100]}
              >
                Revenue Generated
              </Typography>
              <Typography
                variant={isSmallScreen ? "h6" : "h3"}
                fontWeight="bold"
                color={colors.greenAccent[500]}
              >
                $59,342.32
              </Typography>
            </Box>
            <Box>
              <IconButton>
                <DownloadOutlinedIcon
                  sx={{ fontSize: isSmallScreen ? "20px" : "26px", color: colors.greenAccent[500] }}
                />
              </IconButton>
            </Box>
          </Box>
          <Box height={isSmallScreen ? "200px" : "250px"} m={isSmallScreen ? "0" : "-20px 0 0 0"}>
            <LineChart isDashboard={true} />
          </Box>
        </Box>
        <Box
          gridColumn={isSmallScreen ? "span 1" : "span 4"}
          gridRow="span 2"
          backgroundColor={colors.primary[400]}
          overflow="auto"
          p={isSmallScreen ? "10px" : "20px"}
        >
          <Box
            display="flex"
            justifyContent="space-between"
            alignItems="center"
            borderBottom={`4px solid ${colors.primary[500]}`}
            p={isSmallScreen ? "10px" : "15px"}
          >
            <Typography color={colors.grey[100]} variant={isSmallScreen ? "h6" : "h5"} fontWeight="600">
              Recent Transactions
            </Typography>
          </Box>
          {mockTransactions.map((transaction, i) => (
            <Box
              key={`${transaction.txId}-${i}`}
              display="flex"
              justifyContent="space-between"
              alignItems="center"
              borderBottom={`4px solid ${colors.primary[500]}`}
              p={isSmallScreen ? "10px" : "15px"}
            >
              <Box>
                <Typography
                  color={colors.greenAccent[500]}
                  variant={isSmallScreen ? "body2" : "h5"}
                  fontWeight="600"
                >
                  {transaction.txId}
                </Typography>
                <Typography color={colors.grey[100]} variant={isSmallScreen ? "body2" : "body1"}>
                  {transaction.user}
                </Typography>
              </Box>
              <Box color={colors.grey[100]} variant={isSmallScreen ? "body2" : "body1"}>{transaction.date}</Box>
              <Box
                backgroundColor={colors.greenAccent[500]}
                p={isSmallScreen ? "2px 5px" : "5px 10px"}
                borderRadius="4px"
              >
                ${transaction.cost}
              </Box>
            </Box>
          ))}
        </Box>

        {/* ROW 3 */}
        <Box
          gridColumn={isSmallScreen ? "span 1" : "span 4"}
          gridRow="span 2"
          backgroundColor={colors.primary[400]}
          p={isSmallScreen ? "10px" : "30px"}
        >
          <Typography variant={isSmallScreen ? "h6" : "h5"} fontWeight="600">
            Campaign
          </Typography>
          <Box
            display="flex"
            flexDirection="column"
            alignItems="center"
            mt={isSmallScreen ? "10px" : "25px"}
          >
            <ProgressCircle size={isSmallScreen ? "100" : "125"} />
            <Typography
              variant={isSmallScreen ? "h6" : "h5"}
              color={colors.greenAccent[500]}
              sx={{ mt: "15px" }}
            >
              $48,352 revenue generated
            </Typography>
            <Typography variant={isSmallScreen ? "body2" : "body1"}>
              Includes extra misc expenditures and costs
            </Typography>
          </Box>
        </Box>
        <Box
          gridColumn={isSmallScreen ? "span 1" : "span 4"}
          gridRow="span 2"
          backgroundColor={colors.primary[400]}
          p={isSmallScreen ? "10px" : "30px"}
        >
          <Typography
            variant={isSmallScreen ? "h6" : "h5"}
            fontWeight="600"
            sx={{ paddingBottom: isSmallScreen ? "10px" : "30px" }}
          >
            Sales Quantity
          </Typography>
          <Box height={isSmallScreen ? "200px" : "250px"} mt={isSmallScreen ? "0" : "-20px"}>
            <BarChart isDashboard={true} />
          </Box>
        </Box>
        <Box
          gridColumn={isSmallScreen ? "span 1" : "span 4"}
          gridRow="span 2"
          backgroundColor={colors.primary[400]}
          p={isSmallScreen ? "10px" : "30px"}
        >
          <Typography
            variant={isSmallScreen ? "h6" : "h5"}
            fontWeight="600"
            sx={{ marginBottom: isSmallScreen ? "10px" : "15px" }}
          >
            Geography Based Traffic
          </Typography>
          <Box height={isSmallScreen ? "150px" : "200px"}>
            <GeographyChart isDashboard={true} />
          </Box>
        </Box>
      </Box>
    </Box>
  );
};

export default Dashboard;
